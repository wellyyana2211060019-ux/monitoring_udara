...
onValue(ref(db, "sensor"), snap => {
  ...
  if (!d) {
    ...
    airStatus.textContent = "Menunggu Data...";
    airStatus.className = "";
...
/* =============================
   METRIC SELECTION
   (Show/Hide Datasets & Highlight Card)
============================= */
window.selectMetric = (metric) => {
  ...
  // 5. Hide other datasets
  // Mapping metric -> dataset index
...
setInterval(() => {
  ...
  // Keep up to 2000 points (history)
  if (trendChart.data.labels.length > 2000) {
    trendChart.data.labels.shift();
    trendChart.data.datasets.forEach(ds => ds.data.shift());
  }

  // Sliding Window: Always show last 10 seconds
  const lastTime = now.getTime();
  const tenSecondsAgo = lastTime - 10000; // 10 seconds window

  trendChart.options.scales.x.min = tenSecondsAgo;
  trendChart.options.scales.x.max = lastTime;
